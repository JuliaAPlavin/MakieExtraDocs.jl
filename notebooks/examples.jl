...
# ╔═╡ fc701665-1288-4bb0-bd2f-2b7ba0f169e4
using Animations
...
# ╔═╡ 2622eabd-3552-4cf9-b636-c18f22f9ad5c
using Uncertain

# ╔═╡ dd5304a5-2b73-43c5-ab46-89ac85615976
...
let
	...
	Record(lim, (@p maprange(log, 100, 5, length=200) [__; reverse(__)]))
end
...
md"""
Whenever one axes is a zoomed-in version of another _(they can have different content)_, it may be useful to display zoom lines and rectangle to guide the eye. \
This is done with `zoom_lines!(ax1, ax2)` function. It uses axes relative positions and their limits to properly draw lines and rectangles:
"""
...
let
	...
	Record(w, (@p maprange(log, 150, 0.5, length=120) [__; reverse(__)]))
end

# ╔═╡ 2ca62b2e-0947-43cb-81d5-839a85e201c4
let
	fig, ax1, plt = heatmap(rand(100, 100), alpha=0.3, axis=(limits=(@lift (45±5, 80±5)), xreversed=true, yreversed=true))
	ax2, plt = heatmap(fig[1,2], rand(100, 100), alpha=0.3, axis=(;xreversed=true, yreversed=true, xscale=log10))
	Label(fig[0,:], "with Reversed and logscales")
	zoom_lines!(ax1, ax2)
	fig
end
...
let
	w = Observable(5.)
	t = Observable(0.)
	lb = @lift 300 .* sincos($t) .+ (300, 325)
	fig = Figure(size=(700, 700))
	ax1 = Axis(fig, xreversed=true, bbox=BBox(250, 450, 250, 450), limits=(@lift (45±$w, 80±$w)))
	plt = heatmap!(ax1, rand(100, 100), alpha=0.3)
	ax2 = Axis(fig, xreversed=true, bbox=@lift BBox($lb[1], $lb[1]+200, $lb[2], $lb[2]+150))
	plt = heatmap!(ax2, rand(100, 100), alpha=0.3)

	zoom_lines!(ax1, ax2)
	resize_to_layout!(fig)

	Record(t, (@p range(0, 2π, length=200)), framerate=40)
end
...
md"""
`FPlot` is a major and relatively self-contained part of `MakieExtra` that is documented in a separate notebook. [Go to `FPlot` docs](https://juliaaplavin.github.io/MakieExtraDocs.jl/notebooks/fplot.html).
"""
...
let
	fig = Figure(markersize=15)
	
	arrowlines(fig[1,1], [(0, 0), (1, 0.5)])
	arrowlines!([(1.3, 0), (0.9, 0.7)], arrowstyle="<|..>")
	arrowlines!(0..1, x -> sin(5x), arrowstyle=">--|>")
	
	arrowlines(fig[2,1], [(0.1, 0), (10, 0.5)], axis=(;xscale=log10))
	arrowlines!([(1.3, 0), (0.9, 0.7)], arrowstyle="<|..>")
	arrowlines!(0.1..10, sin, arrowstyle=">--|>")
...
Aside from `Scatter + Lines`, common usecases include: `Band + Rangebars`, `Lines + Band`, `Scatter + Rangebars`, `Hist + VLines`, multiple instances of the same recipe with different options, and more.
"""

# ╔═╡ 64bf16ee-67ac-4ec8-9dd3-319b637dd41a
let fig = Figure(size=(1200, 550))

	multiplot(fig[1,1], (Scatter, Lines), rand(100), axis=(;title="Scatter + Lines, same as scatterlines"))

	multiplot(fig[1,2], (Band, Rangebars), 1:20, x -> sin(x) ± abs(cos(x)), axis=(;title="Band + Rangebars, for Intervals"))

	# common attributes are passed to all recipes that support them:
	multiplot(fig[2,1], (Lines, Band), 1:0.2:20, x -> sin(x) ±ᵤ abs(cos(x)), color=(:orange, 0.6), linewidth=3, axis=(;title="Lines + Band, for Uncertain values"))

	# attributes can also be specified for individual recipes:
	multiplot(fig[1,3], (Scatter => (;color=:black, markersize=10), Scatter => (;color=:lightgreen, markersize=5)), randn(100), randn(100), axis=(;title="Scatter × 2"))

	multiplot(fig[2,2], (Hist, VLines => (ymin=0, ymax=0.04, linewidth=0.5, color=:black)), randn(300), axis=(;title="Hist + VLines"))
	multiplot(fig[2,3], (Hist, HLines => (xmin=0, xmax=0.04, linewidth=0.5, color=:black)), randn(300), direction=:x, axis=(;title="Hist + HLines"))
...
let
	...
	linesglow!(0..5, (@lift x->-sin(x^2 + $a)), color=:black, glowwidth=30, glowcolor=(@lift ($c2, 0.4)), label="B")
...
md"""
Here, we provide the `SymLog` scale that uses the same definition as `matplotlib` does, and the `AsinhScale` that is similar to symlog but smooth everywhere:
"""

# ╔═╡ 3c0742fa-6a95-41bf-b1cd-4e2b5a07f10e
X = exp.(10 .* rand(100, 100));

# ╔═╡ 2393a550-8abd-4d3d-9b61-f7df039b79b8
let
	fig = Figure()
	linscale = Observable(1.)
	
	xs = range(0±1e4, length=1000)

	i=1
	scale = identity
	_, plt = image(fig[1,i], X; colorscale=scale, colormap=:turbo, axis=(width=300, height=250, title = "colorscale=identity"))
	Colorbar(fig[2,i], plt, vertical=false)
	lines(fig[0,i], xs, xs; axis=(width=300, height=250, title="yscale=identity", yscale=scale))
	
	for (i, colorscale_) in [SymLog, AsinhScale] |> enumerate
		scale = (@lift colorscale_($linscale))
		_, plt = image(fig[1,1+i], X; colorscale=scale, colormap=:turbo, axis=(width=300,))
		Colorbar(fig[2,1+i], plt, vertical=false)
		lines(fig[0,1+i], xs, xs; axis=(width=300, title=(@lift string($scale)), yscale=scale))
	end

	resize_to_layout!(fig)
	fig
	Record(fig, (@p maprange(log, 1, 1e4, length=60) [__; reverse(__)])) do x
		linscale[] = round(x, sigdigits=2)
	end
end

# ╔═╡ 81dfbfbd-4b94-46ed-a477-5e7f284dc50b
let
	fig = Figure()
	linscale = Observable(1.)
	...
	i=1
	scale = identity
	lines(fig[1,i], xs, xs; axis=(width=300, height=250, title="yscale=identity", yscale=scale))
	
	for (i, colorscale_) in [SymLog, AsinhScale] |> enumerate
		scale = (@lift colorscale_($linscale))
		lines(fig[1,1+i], xs, xs; axis=(width=300, title=(@lift string($scale)), yscale=scale))
	end

	resize_to_layout!(fig)
	Record((@p maprange(log, 1, 1e4, length=60) [__; reverse(__)])) do x
		linscale[] = round(x, sigdigits=2)
	end
end
...
md"""
Makie contains several tick locators/formatters itself, here we provide a few more.

Look at the y-axis ticks in these examples:
"""

# ╔═╡ 01d80794-c229-46c6-b2fb-db4f7e54ad9c
with_theme(Axis=(; yminorticksvisible=true)) do
	fig = Figure()

	scatter(fig[1,end+1], X[1,:], axis=(yscale=log10, width=300, title="Default"))
	scatter(fig[1,end+1], X[1,:], axis=(yscale=log10, yticks=BaseMulTicks([1, 3]),yminorticks=BaseMulTicks(2:9), width=300, title="BaseMulTicks"))
	scatter(fig[1,end+1], X[1,:], axis=(yscale=log10, yticks=BaseMulTicks([1, 3]),yminorticks=BaseMulTicks(2:9), ytickformat=EngTicks(), width=300, title="BaseMulTicks + EngTicks"))
	scatter(fig[1,end+1], X[1,:], axis=(yscale=log10, yticks=BaseMulTicks([1, 2, 5]),yminorticks=BaseMulTicks(2:9), ytickformat=EngTicks(:symbol), width=300, title="BaseMulTicks + EngTicks(:symbol)"))
...
let
	...
	w = Observable(1.)
	n = 30
	scatter!(rand(n), rand(n), marker=:vline, markersize=30, label="Native Makie :vline")
	...
	r = Record(fig, range(0, 1, length=100), framerate=20) do t
		w[] = Animation(
			...
			Animations.linear(postwait=0.3),
			0.65, 1.3,
			Animations.linear(postwait=0.1),
			1., 0.1,
		)(t)
	end
end
...
Concatenation and interpolation is supported – often more readable than using the `Makie.rich(...)` function:
```julia
# plain Makie:
rich("a", subscript("x"), ": ", rich(a, font=:bold), ", b: ", rich(b, font=:italic))
# with MakieExtra, concatenation:
...
let
	a, b = "val1", "val2"
	fig = Figure(size=(400, 50))
	Label(fig[1,1], @rich "a$(subscript("x")): $(rich(a, font=:bold)), b: $(rich(b, font=:italic))")
	fig
end
...
# ╔═╡ ab706086-0258-4d08-94bc-b74aad1c6f92
html"""<style>
body main {
	max-width: 850px !important;
	margin-right: max(max(0px,(100% - (850px + 25px + 6px))/2),min((100% - (850px + 25px + 6px)),500px)) !important;
}
</style>"""
...
manifest_format = "2.0"
project_hash = "0dc4845707694644901811d3c9a8ffd8ee27263b"
...
git-tree-sha1 = "93f595433f1082771c7b367cf027fc6e9c5b04a8"
...
git-tree-sha1 = "30fb8e5e896100404d2af5721b6378e467b5009d"
...
git-tree-sha1 = "a3ba6510dccd2b7a14c42803c1a1d0e971a4c87a"
...
[[deps.MakieExtra]]
deps = ["AccessorsExtra", "DataManipulation", "DataPipes", "InverseFunctions", "KwdefHelpers", "LinearAlgebra", "Makie", "NonNegLeastSquares", "ObjectiveC", "PyFormattedStrings", "Reexport", "StructHelpers"]
git-tree-sha1 = "a26891d34164a451f54772143c0ea507b38ded76"
...
[[deps.Skipper]]
git-tree-sha1 = "725fc552fd61f56990db3ab30903b94fd16d9182"
...
[[deps.StructHelpers]]
deps = ["ConstructionBase"]
git-tree-sha1 = "f637761dde182277be5bb736d5be5a5a2a3f4ed2"
...
[[deps.Uncertain]]
deps = ["Accessors"]
git-tree-sha1 = "3180f981d2edd42b247cc1754df2a3c42fc48f33"
